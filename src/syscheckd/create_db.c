...
static int read_file(const char *file_name, int opts, OSMatch *restriction, whodata_evt *evt)
{
...
#endif
    {
        ...
        if (!buf) {
            ...
            snprintf(alert_msg, OS_MAXSTR, "%c%c%c%c%c%c%c%c%c%ld:%d:%d:%d:%s:%s:%s:%s:%s:%ld:%ld",
                     opts & CHECK_SIZE ? '+' : '-',
                     opts & CHECK_PERM ? '+' : '-',
                     opts & CHECK_OWNER ? '+' : '-',
                     opts & CHECK_GROUP ? '+' : '-',
...
int read_dir(const char *dir_name, int opts, OSMatch *restriction, whodata_evt *evt)
{
    ...
    if (opts & CHECK_WHODATA) {
        realtime_adddir(dir_name, 1);
    } else if (opts & CHECK_REALTIME) {
#ifdef INOTIFY_ENABLED
        realtime_adddir(dir_name, 0);
#else
#ifndef WIN32
